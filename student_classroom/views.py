...
def answerdoubt(request):
    if request.method == "POST":
        ...
        if role == "Teacher":
            status = "Verified"
        obj = Answers(user_id=Users(user_id), classroom_id=Classrooms(class_id), doubt_id=Doubts(doubt_id), ans=ans, validate=status)
        obj.save()
    return DoubtPage(request)
...
def validateanswer(request):
    ...
    if request.method == "POST":
        ...
        status = "Verified"
        obj = Answers.objects.get(answer_id=answer_id)
        obj.validate = status
        obj.save()
        class_id = request.session['class_id']
        doubt_id = request.POST['doubt_id']
